...
public class CheckEmulator {
    ...
    public static boolean isSimulator(Context paramContext) {
        if (isInit) {
            isSimulator = cacheSimulator(paramContext);
            isInit = false;
        }
        return isSimulator;
    }

    private static boolean cacheSimulator(Context context) {
        try {
            List pathList = new ArrayList();
            pathList = getInstalledSimulatorPackages(context);
            if (pathList.size() == 0) {
    ...
    private static List getInstalledSimulatorPackages(Context context) {
        ...
        try {
            for (int i = 0; i < PKG_NAMES.length; i++)
                try {
                    context.getPackageManager().getPackageInfo(PKG_NAMES[i], GET_ACTIVITIES);
