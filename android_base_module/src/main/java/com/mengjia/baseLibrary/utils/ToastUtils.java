...
public class ToastUtils {
    ...
    private static boolean initViewToast;
    ...
    public static void setView(@LayoutRes final int layoutId) {
        sHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (instance != null) {
                    LayoutInflater inflate = (LayoutInflater) instance.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    public static void setFontFamily(Typeface typeface) {
    ...
    private static void show(@StringRes int resId, int duration) {
        show(LanguageManager.getInstance().getCurrentLocaleRes().getString(resId), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(LanguageManager.getInstance().getCurrentLocaleRes().getString(resId), args), duration);
    }
     ...
     * @param duration 显示时长
     */
    // TODO: 2021/9/30 内存泄漏优化
    private static void show(CharSequence text, int duration) {
        cancel();
        boolean isCustom = false;
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference;
            ImageView imgToastBg = (ImageView) ((ViewGroup) view).getChildAt(0);
            TextView tvMessage = (TextView) ((ViewGroup) view).getChildAt(1);
//            if ( !initViewToast) {
            sToast = new Toast(BaseApp.getInstance());
            int screenWidth = ScreenUtil.getScreenWidth(BaseApp.getInstance());
            int width = AppUtils.isPortrait() ? screenWidth : (int) (screenWidth * 0.5);
            FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(width, ViewGroup.LayoutParams.MATCH_PARENT);
            int toast_bg = ResourcesUtil.getMipmapId("toast_bg");
            imgToastBg.setImageResource(toast_bg);
            int toast_message_text_color = ResourcesUtil.getColorIdRes("toast_message_text_color");
            tvMessage.setTextColor(toast_message_text_color);
...
//            }
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(BaseApp.getInstance(), spannableString, duration);
            } else {
